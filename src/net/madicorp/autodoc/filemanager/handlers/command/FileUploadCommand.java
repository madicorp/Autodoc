...
import net.madicorp.autodoc.filemanager.configuration.Constants;
import net.madicorp.autodoc.filemanager.configuration.IConfiguration;
import net.madicorp.autodoc.filemanager.configuration.Events.EventTypes;
import net.madicorp.autodoc.filemanager.data.AfterFileUploadEventArgs;
import net.madicorp.autodoc.filemanager.data.ResourceType;
import net.madicorp.autodoc.filemanager.errors.ConnectorException;
import net.madicorp.autodoc.filemanager.errors.ErrorUtils;
import net.madicorp.autodoc.filemanager.utils.AccessControlUtil;
import net.madicorp.autodoc.filemanager.utils.FileUtils;
import net.madicorp.autodoc.filemanager.utils.ImageUtils;
...
public class FileUploadCommand extends Command implements IPostCommand {
	...
	public void execute(final OutputStream out) throws ConnectorException {
		...
		try {
			String errorMsg = this.errorCode == Constants.Errors.CKFINDER_CONNECTOR_ERROR_NONE ? "" :                            
				( this.errorCode == Constants.Errors.CKFINDER_CONNECTOR_ERROR_CUSTOM_ERROR ? this.customErrorMsg :
	...
	private String getFileItemName(final FileItem item) {
		...
		String prev = (m.find()) ? m.group(0) : "";
		if(prev!="")
		{
		String [] tprev = prev.split("\\.");
		String name = tprev[0]+"_"+Dates.date().getTime()+"."+tprev[1];
		return name;
		}
		else
		{
