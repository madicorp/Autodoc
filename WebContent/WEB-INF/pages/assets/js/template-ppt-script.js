...
$(function(){
	...
	$("form").submit(function(e){
		  ...
		  var extention = t[t.length-1];
			if(extention != "pptx" && extention != "potx")
				{
					$(".empty-alert").show();
					 $(".loading").hide();
				}
		  ...
		  $.ajax({
		    url: 'connector?command=FileUpload&type=TemplatePpt',
		    type: 'POST',
		    data: formData,
		    async: false,
		    cache: false,
    ...
    $(".rename-conf").click(function(){
   	   ...
   	   $.ajax({
  			 ...
  			 type:"put",
  				url:"template?type=Ppt&templateref="+template+"&templatename="+name,
  				success: function(data){
  					if (data)
  						{
  							location.href = "template-ppt";
    ...
    $(".delete-conf").click(function(){
 	   var gabarit = $("#del-template-ref").val();
 	   var loading = $(this).next(".loading");
 	   loading.show();
 	   ...
 	   $.ajax({
			 ...
			 type:"delete",
				url:"template?templateref="+gabarit+"&type=Ppt",
				success: function(data){
					if (data)
						{
							location.href = "template-ppt";
...
function load(){
	 $.ajax({
		 type:"get",
			url:"connector",
			data:"command=GetFiles&type=TemplatePpt",
			dataType: 'xml',
			success: function(data){
				if (data)
					{
					...
					$(d).find("files").children().each(function(){
						...
						$('#dataTables > tbody').find("tr").each(function(){
							...
							if(cls.contains(fileName))
								...
								$(this).find(".name").html("<a href='connector?command=DownloadFile&type=TemplatePpt&FileName="+fileName+"'>"+newName+"</a>");
								return;
								}
