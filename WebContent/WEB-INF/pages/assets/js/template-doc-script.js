...
$(function(){
	...
	$("form").submit(function(e){
		  ...
		  var extention = t[t.length-1];
			if(extention != "docx" && extention != "docm" && extention != "dotx" && extention != "dotm")
				{
					$(".empty-alert").show();
					 $(".loading").hide();
				}
		  ...
		  $.ajax({
		    url: 'connector?command=FileUpload&type=TemplateDoc',
		    type: 'POST',
		    data: formData,
		    async: false,
		    cache: false,
    ...
    $(".rename-conf").click(function(){
   	   ...
   	   $.ajax({
  			 ...
  			 type:"put",
  				url:"template?type=Docx&templateref="+template+"&templatename="+name,
  				success: function(data){
  					if (data)
  						{
  							location.href = "template-doc";
    ...
    $(".delete-conf").click(function(){
 	   ...
 	   $.ajax({
			 ...
			 type:"delete",
				url:"template?templateref="+template+"&type=Docx",
				success: function(data){
					if (data)
						{
							location.href = "template-doc";
...
function load(){
	 $.ajax({
		 ...
		 type:"get",
			url:"connector",
			data:"command=GetFiles&type=TemplateDoc",
			dataType: 'xml',
			success: function(data){
				if (data)
					{
					...
					$(d).find("files").children().each(function(){
						...
						$('#dataTables > tbody').find("tr").each(function(){
							...
							if(cls.contains(fileName))
								...
								$(this).find(".name").html("<a href='connector?command=DownloadFile&type=TemplateDoc&FileName="+fileName+"'>"+newName+"</a>");
								return;
								}
